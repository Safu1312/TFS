...
//---------------------------------------issue fine code goes here ---------------------------------------------------------------
    ...
    $driver_email = $_POST['driveremail'] ?? ''; // Get the driver email from the form submission
    ...
    else
     {
		...
		if ($result2) {
            ...
            try {
                ...
                //Recipients
                $mail->setFrom('tfms.srilanka@gmail.com', 'Traffic Police Sri Lanka');
                $mail->addAddress($driver_email, $res['driver_name']); // Add a recipient        
                // Content
                ...
                <head>
                    ...
                    <style>
                        ...
                        <body>
                            <div class="container">
                                <div class="header">
                                    <h1>Traffic Police Notification</h1>
                                </div>
                                <div class="content">
                                    <h2>Dear ' . htmlspecialchars($drivername) . ',</h2>
                                    <p>You have been issued a fine for the following offense:</p>
                                    <ul>
                                        <li><strong>License ID:</strong> ' . htmlspecialchars($license) . '</li>
                                        <li><strong>Vehicle No:</strong> ' . htmlspecialchars($vehicleno) . '</li>
                                        <li><strong>Place:</strong> ' . htmlspecialchars($place) . '</li>
                                        <li><strong>Fine Amount:</strong> ' . htmlspecialchars($fineamount) . '</li>
                                    </ul>
                                    <p>Please pay the fine by the due date to avoid any penalties.</p>
                                    <a href="http://yourpaymentlink.com" class="button">Pay Fine Now</a>
                                </div>
                                <div class="footer">
                                    <p>Thank you for your cooperation.</p>
                                    <p>&copy; ' . date("Y") . ' Traffic Fine Management System Sri Lanka. All rights reserved.</p>
                                </div>
                            </div>
                        </body>
                        </html>
                ...
                $mail->send();
                header("Location: add_new_fine.php?success=Added Successfully and email sent");
                exit();
            } catch (Exception $e) {
                header("Location: add_new_fine.php?error=Fine added but email could not be sent. Mailer Error: {$mail->ErrorInfo}");
                exit();
            }
        } else {
    ...
    <div class="dashwrapper animated fadeIn">
        <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12 col-xl-12">
            <div class="container-fluid">
                ...
                <input type="email" class="form-control" id="driver_email" value="<?php echo $res['driver_email'] ?? ''; ?>" name="driveremail" placeholder="Driver Email" disabled>
                <input type="hidden" name="driveremail" value="<?php echo $res['driver_email'] ?? ''; ?>">
                </div>
                    <div class="form-group col-md-6">
